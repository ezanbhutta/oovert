...
function initEnquiryRibbon() {
  ...
  const update = () => {
    ...
    // Present from the top and all the way down, so the ask is never more than
    // one tap away. It is a fixed element, so the hero reserves clearance for
    // it on narrow and portrait screens (see .hero in sections.css) rather than
    // letting it land on the copy.
